...
export default function PatternPracticeModal({ pattern, onClose }) {
  const navigate = useNavigate();
  const fetchConceptPracticeSet = useConceptStore((state) => state.fetchConceptPracticeSet);
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(true);

  const patternId = `pattern:${pattern.chapterId}:${pattern.key}`;

  useEffect(() => {
    let active = true;
    const loadData = async () => {
      setLoading(true);
      const res = await fetchConceptPracticeSet(patternId);
      if (active && res) {
        setData(res);
      }
      if (active) {
  ...
  }, [patternId, fetchConceptPracticeSet]);

  const handleStart = () => {
    navigate(`/practice/${patternId}`);
    onClose();
  };
      ...
      <div className="w-full max-w-md rounded-2xl border border-border-default bg-bg-surface/95 shadow-xl p-6 relative flex flex-col gap-4 overflow-hidden z-10">
        <div>
          ...
          <p className="text-[12px] text-text-secondary leading-relaxed">
            Practice mode generates a focused session of 5 questions, prioritized to help you master this pattern group (Unsolved → Revisit → Done).
          </p>
        </div>
        ...
        <div className="border-y border-border-default/40 py-4 flex flex-col gap-3 min-h-[160px] justify-center">
          {loading ? (
            ...
            <div className="text-center py-6 text-xs text-text-muted">
              No questions found for this pattern group.
            </div>
          ) : (
