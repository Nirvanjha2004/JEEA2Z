...
const concepts = [
  {
    name: "Newton's Laws & FBD",
    slug: 'newtons-laws-fbd',
    description: "Drawing free body diagrams, resolving forces, and applying Newton's laws to systems in equilibrium or acceleration.",
    formula_ids: [],
    pattern_group: 'laws-fbd'
  },
  {
    name: 'Pulley Systems & Constraint Motion',
    slug: 'pulley-constraints',
    description: 'Finding tension and acceleration in fixed and movable pulley systems using string constraints.',
    formula_ids: [],
    pattern_group: 'pulley-constraints'
  },
  {
    name: 'Friction (Static & Kinetic)',
    slug: 'friction-dynamics',
    description: 'Analyzing static and kinetic friction forces on horizontal and inclined surfaces, and multi-block systems.',
    formula_ids: [],
    pattern_group: 'friction'
  },
  {
    name: 'Circular Motion Dynamics',
    slug: 'circular-dynamics',
    description: 'Dynamics of circular motion, centripetal force, and banking of roads.',
    formula_ids: [],
    pattern_group: 'circular-dynamics'
  },
  {
    name: 'Pseudo Forces & Non-Inertial Frames',
    slug: 'pseudo-forces',
    description: 'Analyzing motion from accelerating reference frames using pseudo forces.',
    formula_ids: [],
    pattern_group: 'pseudo-forces'
  },
  {
    name: 'Impulse and Variable Mass',
    slug: 'impulse-momentum',
    description: "Force from momentum changes, impulse-momentum theorem, and variable mass systems like rocket propulsion.",
    formula_ids: [],
    pattern_group: 'impulse-momentum'
  }
];

async function seed() {
  try {
    console.log('Starting Laws of Motion Seeding...');
    
    console.log('Clearing old questions for Chapter 3...');
    await pool.query('DELETE FROM questions WHERE chapter_id = 3');
    ...
    for (const c of concepts) {
      const res = await pool.query(
        `INSERT INTO concepts (chapter_id, name, slug, description, formula_ids, pattern_group)
         ...
         RETURNING id, slug`,
        [3, c.name, c.slug, c.description, c.formula_ids, c.pattern_group]
      );
      conceptMap[res.rows[0].slug] = res.rows[0].id;
    }
    
    console.log('2. Reading questions list from JSON...');
    const questionsPath = path.join(__dirname, 'laws_of_motion_questions.json');
    ...
    console.log('Laws of Motion seeding completed successfully!');
    process.exit(0);
  } catch (err) {
